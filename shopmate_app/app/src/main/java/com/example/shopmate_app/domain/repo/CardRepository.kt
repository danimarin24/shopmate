...
class CardRepository @Inject constructor(private val api: CardService){
    ...
    suspend fun generateCardShareLinkToken(shareReq: CardShareLinkRequestEntity): CardShareLinkResponseEntity {
        val response = api.generateCardShareLinkToken(shareReq)
        CardProvider.shareCardLink = response
        return response
    }
    ...
    suspend fun addItemToACard(item: ItemCardLineEntity): ItemCardLineEntity {
        val response = api.addItem(item)
        ItemProvider.item = response
        return response!!
    }

    suspend fun removeItemFromACard(cardId: Int, itemId: Int): ItemCardLineEntity {
        val response = api.removeItem(cardId, itemId)
        ItemProvider.lastItemRemoved = response
        return response!!
    }
